...
class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def create_non_admin
    ...
    Rails.cache.write('hola','lunes');
    railsReturn = Rails.cache.fetch('hola');
    if non_admin.save
      UserMailer.non_admin_registration(non_admin).deliver
      # logger.info "Successfully added a non admin user: #{non_admin.email}"
      flash['notice'] = "Successfully added a non admin user: #{railsReturn}"
    else 
  ...
  def configure_sign_up_params
    devise_parameter_sanitizer.permit(:sign_up, keys: [:name, :lastname, :organization_id])
  end
